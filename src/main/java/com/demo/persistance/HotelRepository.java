...
public interface HotelRepository extends PagingAndSortingRepository<Hotel, Long> {
     ...
     * <pre>
     *     Example: state = null
     *     1. where upper(h.address.state) = coalesce(null, upper(h.address.suburb))
     *     2. where upper(h.address.state) = upper(h.address.suburb) // -> row will match
     *
     *     Example: state != null
     *     1. where upper(h.address.state) = coalesce(VIC, upper(h.address.suburb))
     *     2. where upper(h.address.state) = VIC // the state will now be used in the query.
     * </pre>
     */
